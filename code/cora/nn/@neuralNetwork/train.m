...
loss = struct('center',zeros(numIter,1),...
    ...
    ...'noiseScale',zeros(length(numIter),1),...
    ...'approxErrRel',zeros(length(numIter),1),...
);
...
if strcmp(options.nn.train.method,'set')
    ...
    % idBatch = cast(repmat(idMat,1,1,min(miniBatchSize,propBatchSize)), ...
    %     'like',inputDataClass);
end
...
for epoch=1:maxEpoch % epoch
    ...
    for i=1:miniBatchSize:max(1,N - mod(N,miniBatchSize)) % training iterations
        ...
        elseif strcmp(options.nn.train.method,'set')
            ...
            for j=1:propBatchSize:...
                ...
                lossBatch.total = lossBatch.total + ...
                    lossScale*lossBatch_.total;
                % lossBatch.approxErrRel = lossBatch.approxErrRel + ...
                %     length(propIdx)/length(miniBatchIdx)*lossBatch_.approxErrRel;
            end
            % if iterScale > 0
            %     noiseScale = (1 - 1e-4)*noiseScale + 1e-4*lossBatch.approxErrRel;
            %     loss.approxErrRel(iter) = lossBatch.approxErrRel;
            % end
        else
        ...
        loss.total(iter) = loss.total(iter) + lossBatch.total;
        ...
        if ismember(iter,valIter) 
            ...
            for j=1:miniBatchSize:max(1,valN - mod(valN,miniBatchSize))
                ...
                lossValTotal(k) = lossValBatch.total*length(valBatchIdx);
...
if options.nn.interval_center && ndims(cy) == 3
    ...
    % if options.nn.train.ibp_center_loss
    %     % Gowal-IBP Loss
    %     z = t.*cyl + (1-t).*cyu;
    ...
    % Compute interval-loss of approx. error and nominal set.
    % approxErrRel = 2*reshape(sum(abs(Gy),[1 2]),[1 batchSize])./max(sum(cyu - cyl),1e-8);
    % approxErrRel(isnan(approxErrRel)) = inf;
    % approxErrRel = min(approxErrRel,1);
    % loss.approxErrRel = sum(approxErrRel,'all')/batchSize;
else
    % Use center.
    z = cy;
    % Compute loss of center.
...
if any(noise > 0)
    if strcmp(initGens,'l_inf')
        ...
        % nnzIdx = reshape(sub2ind(size(GBatch),idx1,idx1,idx2), ...
            % [v0 batchSize]);
        % GBatch(nnzIdx) = noise.*reshape(GBatch(nnzIdx),[v0 batchSize]);
    ...
    elseif strcmp(initGens,'sensitivity')
        ...
        % GBatch = reshape(idMat(idx{:}), [v0 numInitGens entsPerGen batchSize]);
        % GBatch = reshape(sum(GBatch,3),[v0 numInitGens batchSize]);
        % GBatch = reshape(idMat(:,idx(1:numInitGens,:)),[v0 numInitGens batchSize]);
        % Scale the generators by the current training noise.
        % GBatch = permute(noise,[1 3 2]).*GBatch;
        ...
        if options.nn.interval_center
            % idx_ = sub2ind([v0 batchSize],idx(:,numInitGens+1:end,:),permute(repelem(1:batchSize,v0 - numInitGens,1),[3 1 2]));
...
function aux_printTrainingIteration(options,table,iter,epoch,trainTime,valIter,loss)
    ...
    if strcmp(options.nn.train.method,'set') && ~strcmp(options.nn.train.loss,'custom')
        ...
        cvalues = {epoch,iter,datetime(trainTimeVec,'Format','HH:mm:ss'),...
            loss.total(iter),loss.valTotal(valIterIdx)};
    end
