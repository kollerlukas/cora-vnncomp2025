...
function [genIds,numGen,numNeurons] = aux_prepareLayers(layers,idxLayer, ...
    numApproxErr,genIds,numGen,numNeurons)
    ...
    for i=idxLayer
        ...
        if isa(layeri,'nnCompositeLayer')
            ...
            layeri.backprop.store.genIds = genIds;
            ...
            for j=1:length(layeri.layers)
                ...
                [genIdsij,numGen,numNeurons] = aux_prepareLayers( ...
                    layersij,idxLayerij,numApproxErr,genIdsij, ...
                        numGen,numNeurons);
                % Add new generators.
            ...
            % neurons.
            [genIds,numGen,numNeurons] = aux_prepareLayer(layeri, ...
                numApproxErr,genIds,numGen,numNeurons);
        end
    end
end

function [genIds,numGen,numNeurons] = aux_prepareLayer(layeri, ...
    numApproxErr,genIds,numGen,numNeurons)
    % Store number generators of the input
    layeri.backprop.store.genIds = genIds;
    ...
    elseif isa(layeri,'nnActivationLayer')
        if numApproxErr > 0 % && ~options.nn.interval_center
            ...
            layeri.backprop.store.approxErrGenIds = approxErrGenIds;
